...
- **db/**: Database layer using GORM with PostgreSQL
  - **connector_registrations.go**: Manages connector registration records in the database
...
- **lib/**: Core business logic for connector validation and database operations
  - **updateConnectorInDB.go**: Updates connector data in the database
  - **validateConnectorSystemToken.go**: Validates the system token for connector operations
  - **validateOperationToken.go**: Validates tokens for data operations
- **models/**: Data structures and application models
  - **app.go**: General application models
  - **connectorDetails.go**: Models for connector details
  - **dynamicField.go**: Models for dynamic form fields
  - **validationResponse.go**: Models for validation responses
- **utils/**: Utility functions for environment loading, token generation, and parsing
  - **constants.go**: Application-wide constants
  - **findProjectRoot.go**: Utility to find the project root directory
  - **loadEnvs.go**: Environment variable loading utility
  - **parseFormFields.go**: Form field parsing utility
  - **parseHeaders.go**: Header parsing utility
  - **parseQueryParams.go**: Query parameter parsing utility
  - **pathComponents.go**: Path manipulation utility
  - **token.go**: Token generation and validation utilities
