...
func DeleteSubscriptionsByOperationID(operationID uint) error {
	if err := DB.Where("operation_id = ?", operationID).Delete(&Subscription{}).Error; err != nil {
...
func DeleteSubscriptionsByConnectorRegistrationID(connectorRegistrationID uint) error {
	if err := DB.Where("connector_registration_id = ?", connectorRegistrationID).Delete(&Subscription{}).Error; err != nil {
		return fmt.Errorf("failed to delete subscriptions: %w", err)
	}
	return nil
}
