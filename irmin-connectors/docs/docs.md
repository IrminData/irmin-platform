...
- [func HandleOperationSchemaGet\(c fiber.Ctx, provider SchemaOperationProvider, logger \*slog.Logger, dbInstance \*db.Database, app \*models.ConnectorsApp\) error](<#HandleOperationSchemaGet>)
...
Under the unified Guard API the push handler is synchronous but still persists an OperationJob row via manager.Begin so concurrent pushes against the same operation get a structured 409 carrying the blocking job\_id. Release is deferred; a provider panic is recovered so the row never sticks at status=running.

<a name="HandleOperationSchemaGet"></a>
## func HandleOperationSchemaGet

```go
func HandleOperationSchemaGet(c fiber.Ctx, provider SchemaOperationProvider, logger *slog.Logger, dbInstance *db.Database, app *models.ConnectorsApp) error
```
...
RegisterJobHandlers mounts the top\-level async\-job HTTP endpoints on the provided fiber app. Called from main at startup, once. Routes are top\-level \(not under a connector slug\) because the SDK client addresses them by job\_id alone — the connector slug is a server\-side detail that lives inside the persisted OperationJob row.
...
All routes require the operation token associated with the job's backing operation row \(Authorization: Bearer \<operation\-token\>\). The job\_id remains opaque, but is no longer an auth secret.
