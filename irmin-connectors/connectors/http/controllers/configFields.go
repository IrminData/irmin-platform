...
// ConfigFields godoc
// @Summary Get HTTP connector configuration fields
// @Description Get dynamic configuration fields for the HTTP connector based on the configuration key (details or settings)
// @Tags http
...
// @Router /http/configuration/{key}/fields [post]
...
func (cs *Controllers) getDetailsFields() map[string]irminmodels.DynamicField {
	return httpconfig.GetDetailsFieldDefinitions()
}
...
func (cs *Controllers) getSettingsFields() map[string]irminmodels.DynamicField {
	return httpconfig.GetSettingsFieldDefinitions()
}
