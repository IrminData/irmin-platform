...
to the Core-side handbook at `core/handbook/connector-architecture.md`,
...
- **Core-side fan-out**: `connectorjobs.ContextWithProgress` wraps
  ...
  `core/orchestrator/executeWorkflowableCommon.go`.
...
- `core/handbook/connector-architecture.md` — the same topics from the
