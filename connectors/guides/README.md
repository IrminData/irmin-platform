...
- **Extending the OAuth infrastructure itself?** See the Core-side code
  under `core/services/oauth/` and `core/controllers/oauth.go` — those
  live alongside their unit tests and don't have a dedicated guide yet.

## Conventions used in these guides

- `Core` means the Irmin Core API project (`core/`).
- `Connectors` (capitalized) means this project (`connectors/`); a
