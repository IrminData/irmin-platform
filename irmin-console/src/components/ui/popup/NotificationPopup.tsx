...
const NotificationPopup = ({
  ...
  const [notifications, setNotifications] = useState<Notification[]>([
    ...
    {
      id: 7,
      title: 'Repository failed to create',
      message: 'Users (public), Repository creation has failed',
      path: ``,
      relatedWorkspaceSlug: workspaceSlug,
      relatedWorkspaceName: workspaceName,
  ...
  return (
    ...
    >
      <div className='max-h-[320px] w-64 overflow-y-scroll rounded-md bg-popover text-popover-foreground shadow-lg'>
        <div className='absolute -top-[16px] left-auto right-[16px] z-50 -translate-x-1/2 transform text-popover md:left-[22px] md:right-auto'>
          <IoTriangle size={20} />
        </div>
        <div className='sticky top-0 z-10 bg-popover py-2 shadow-sm'>
          <div
          ...
          >
            <div className='flex items-center justify-between'>
              ...
              <Button onClick={clearNotifications} variant='link' size='sm'>
          ...
          {notifications.length > 0 ? (
            notifications
              ...
              .map((notification, index) => (
                ...
                >
                  <div
                    className={`border-b px-3 py-2 text-xs text-gray-800 hover:bg-gray-100 dark:text-gray-100 dark:hover:bg-gray-800 ${notification.type === 'info' ? 'border-accent' : ''} ${notification.type === 'warning' ? 'border-yellow-500' : ''} ${notification.type === 'error' ? 'border-red-500' : ''} ${notification.type === 'success' ? 'border-green-500' : ''} `}
                  >
