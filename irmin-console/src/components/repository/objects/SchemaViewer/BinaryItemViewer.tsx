...
export function BinaryItemViewer({
  ...
  const getIcon = () => {
    ...
    if (ct.startsWith('image/'))
      return <MdImage className='h-6 w-6 text-green-500' />;
    if (ct.startsWith('text/'))
      return <MdTextFields className='h-6 w-6 text-orange-500' />;
    return <MdInsertDriveFile className='h-6 w-6 text-gray-500' />;
  };
