...
import { useAllWorkflowRuns, useCancelWorkflowRun } from '@/hooks/api';
...
const AllWorkflowRunsSection = () => {
  ...
  const { currentPage, totalPages, goToPage, allWorkflowRunsQuery } =
    useAllWorkflowRuns();
  const { confirmAndCancel } = useCancelWorkflowRun();
  const canCancelRuns = useMemo(
    () => isResourceAllowed('workflow_run', 'delete'),
    [isResourceAllowed]
  );
  ...
  const runRows: GridRow[] = useMemo(
    () =>
      allWorkflowRunsQuery.data?.data?.map((run: WorkflowRun) => {
        // 'cancelling' is the optimistic state we flip the row to
        // after the user clicks Cancel. Excluding it from the action
        // gate (rather than disabling the button visually) avoids a
        // second confirm dialog while the first cancel is in flight
        // — the row-level TableRowAction has no disabled affordance,
        // so removing the action is the safe equivalent.
        const cancellable =
          run.status === 'pending' ||
          run.status === 'initiating' ||
          run.status === 'running';
        const showCancelAction = canCancelRuns && cancellable;
        return {
          columns: [
            <div
          ...
          actions: [
            ...(showCancelAction
              ? [
                  {
                    label: dict.workflow.cancelRun.label,
                    primary: false,
                    onClick: () => {
    ...
    [
      ...
      locale,
      workspaceUrl,
      allWorkflowRunsQuery.data?.data,
      canCancelRuns,
      confirmAndCancel,
    ]
  );
