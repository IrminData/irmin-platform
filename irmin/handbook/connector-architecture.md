...
`services/oauth/`, `db/oauth.go`, `controllers/oauth.go`, or
...
- `services/oauth/` — flow orchestration: `StartFlow`, `HandleCallback`,
  `GetAccessToken`, `RefreshToken`, `Revoke`.
- `services/oauth/dcr.go` — RFC 7591 Dynamic Client Registration
  (lazy: registers a workspace-scoped client on first use).
- `controllers/oauth.go` — four HTTP surfaces:
...
- `services/oauth/` — the flow orchestration code
