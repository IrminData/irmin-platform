...
| 2 | OAuth flow — endpoints, DCR, tokens, sessions, JIT access-token endpoint | ✅ Shipped (this branch) |
| 3 | Shared OAuth base in `irmin-connectors` | 🚧 Next |
| 4 | Stripe connector (pull · push · patch) | ⏳ Planned |
...
Motivation: an OAuth-backed connector should declare the minimum —
auth URLs, scopes, write-path mapping — and inherit everything else
(token fetch, refresh triggering, 401 retry, schema discovery glue)
from a shared base. This keeps per-connector code small enough that
adding the tenth OAuth connector isn't ten times the work of adding
the first.

**Deliverables**

- `irmin-sdk-go`: add `ConnectionOAuthConfig` to the connector `Info`
  response. Fields: `Provider`, `AuthorizationURL`, `TokenURL`,
  `RevocationURL` (optional), `DCREndpoint` (optional, RFC 7591),
  `Scopes`, `PKCE` (required true), `ExtraParams` (e.g.,
  `access_type=offline`). Ship as a tagged SDK release; Core + the
  connectors service pin to it.
- `irmin-connectors/connectors/common/oauth_base.go` — abstract base
  that wires up:
  - Fetching access tokens from Core's internal endpoint on every
    outbound call.
  - Retry-on-401 with a forced refresh.
  - Surfacing `ConnectionOAuthConfig` through the connector `Info`
    response so the console knows to render a Connect button.
- `irmin-connectors/connectors/common/oauth_client.go` — the HTTP
  middleware itself; shared by every OAuth connector.
- `irmin-connectors/connectors/common/oauth_test_fixture.go` — an
  in-process `httptest.Server` that speaks enough of OAuth +
  DCR + token refresh to drive per-connector integration tests.
- Per-connector boilerplate: a `NewOAuthConnector` constructor that
  takes vendor specifics (base URL, path-to-endpoint mapping,
  schema discovery hook) and returns a ready connector.

**Acceptance criteria**

- `connectors/common/` exposes an embeddable `OAuthConnector` base
  that concrete connectors compose into their own controller. Works
  equally well for static-client and DCR vendors.
- The base handles: operation init/status/cancel lifecycle, the
  `X-Irmin-Connection-Id` read, `lib.OAuthTokenClient` calls,
  sentinel-to-HTTP mapping, audit hooks.
- At least one existing static-credential connector (propose: HTTP,
  the simplest) migrates to the base as a proof — no behavior
  change, just fewer lines of glue.
- The base is covered by unit tests with a mock vendor + in-process
  mock of Core's `/system/oauth/access-token` endpoint.
- A fake vendor connector, written against the base in under ~100
  LOC of vendor-specific code, passes the generic OAuth integration
  test suite.

**Out of scope.** Migrating Postgres / MySQL / SFTP to the new base.
They keep working on the legacy path; opportunistic migration in
later PRs.

**Estimate.** ~500–800 LOC in `connectors/common/`, ~200 in the
ported HTTP connector, plus tests. One focused PR.
