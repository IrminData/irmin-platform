...
// @Success 200 {object} irminmodels.IrminAPIResponse{data=irminmodels.AssistantConversation} "Query generation conversation retrieved successfully"
// @Failure 401 {object} irminmodels.IrminAPIResponse "Unauthorized - invalid or missing authentication"
// @Failure 404 {object} irminmodels.IrminAPIResponse "Conversation not found"
// @Failure 500 {object} irminmodels.IrminAPIResponse "Internal server error"
// @Router /workspaces/{workspace_slug}/assistant/query/{conversation_id} [get]
func (api *APIControllers) QueryGenerationShow(c fiber.Ctx) error {
	...
	// Get the query generation conversation
	queryConversation, err := api.Services.GetQueryGenerationConversation(c, user, workspace, conversation)
	if err != nil {
		api.Logger.Error("Error getting query generation conversation", "error", err)
		return utils.WriteResponse(c, fiber.StatusInternalServerError, irminmodels.IrminAPIResponse{
			Errors: []string{api.lm.T(dict, "error_occurred")},
		})
	}
	...
	// Structure the response.
	conversationResponse, formatErr := formatter.FormatAssistantConversationResponse(queryConversation, api.SQIDManager)
	if formatErr != nil {
		api.Logger.Error("Error formatting query generation conversation", "error", formatErr)
		return utils.WriteResponse(c, fiber.StatusInternalServerError, irminmodels.IrminAPIResponse{
			Errors: []string{api.lm.T(dict, "error_occurred")},
		})
	}
...
// @Param workspace_slug path string true "Workspace slug"
// @Param conversation_id path string true "Conversation ID"
// @Success 200 {object} irminmodels.IrminAPIResponse "Query generation conversation deleted successfully"
// @Failure 401 {object} irminmodels.IrminAPIResponse "Unauthorized - invalid or missing authentication"
// @Failure 404 {object} irminmodels.IrminAPIResponse "Conversation not found"
// @Failure 500 {object} irminmodels.IrminAPIResponse "Internal server error"
// @Router /workspaces/{workspace_slug}/assistant/query/{conversation_id} [delete]
func (api *APIControllers) QueryGenerationDestroy(c fiber.Ctx) error {
	...
	// Delete the query generation conversation
	err = api.Services.DeleteQueryGenerationConversation(c, user, workspace, conversation)
	if err != nil {
		api.Logger.Error("Error deleting query generation conversation", "error", err)
		return utils.WriteResponse(c, fiber.StatusInternalServerError, irminmodels.IrminAPIResponse{
			Errors: []string{api.lm.T(dict, "error_occurred")},
		})
	}
